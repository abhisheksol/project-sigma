...
class UpdateAllocationFileCaseDetails(Utils):
    ...
    def normalize_address_part(self, part: str) -> str:
        """Remove punctuation, trim spaces, and lowercase the address part."""
        return re.sub(r"[^\w\s]", "", part).strip().lower()
    ...
    def update_allocation_file_cases_details(self) -> List[Dict[str, Any]]:
        ...
        areas: List[str] = RegionConfigurationAreaModel.objects.only("id", "title")
        area_lookup: Dict[str, RegionConfigurationAreaModel] = {
            self.normalize_address_part(a.title): a for a in areas
        }
        ...
        for record_idx, record in enumerate(self.file_data):
            ...
            save_sub_area_to_allocation_case_instance(
                case_instance=case_instance, area_lookup=area_lookup
            )
